...
//     pos: START_POS[dir], speed: 3.5 + Math.random() * 1.5, stopped: false,
...
//       });
//     });
//     s.on('available_models', models => setAvailableModels(models));
//     s.emit('get_models');
//     return () => s.close();
//   }, []);
...
//     if (model) socket.emit('select_model', model);
//   };
...
//           <h3 className="panel-title">MODEL BRAIN</h3>
//           <div className="slider-wrapper">
//             <select 
//               className="cyber-select" 
//               value={simState.activeModel || "Internal Q-Learning"} 
...
//             </select>
//           </div>
//           <div style={{ fontSize: '0.65rem', color: '#8A92A6', marginTop: '8px', display: 'flex', alignItems: 'center', gap: '5px' }}>
//             <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#00f2fe' }} />
//             ACTIVE: <span style={{ color: '#00f2fe', fontWeight: 700 }}>{simState.activeModel?.toUpperCase()}</span>
//           </div>
//         </div>
...
const ROAD_NS_LEFT  = CX - ROAD_W / 2;
const ROAD_NS_RIGHT = CX + ROAD_W / 2;
const ROAD_EW_TOP   = CY - ROAD_W / 2;
const ROAD_EW_BOT   = CY + ROAD_W / 2;

const IX1 = ROAD_NS_LEFT, IX2 = ROAD_NS_RIGHT;
const IY1 = ROAD_EW_TOP,  IY2 = ROAD_EW_BOT;

const CAR_W = 24, CAR_H = 40, CAR_GAP = 8;
...
const STOP_POS  = { NS: IY1 - 6, SN: IY2 + 6, EW: IX1 - 6, WE: IX2 + 6 };
const SIGN      = { NS: 1, SN: -1, EW: 1, WE: -1 };
const START_POS = { NS: -CAR_H / 2 - 10, SN: H + CAR_H / 2 + 10, EW: -CAR_H / 2 - 10, WE: W + CAR_H / 2 + 10 };
const END_POS   = { NS: H + CAR_H, SN: -CAR_H, EW: W + CAR_H, WE: -CAR_H };
...
export default function TrafficSimulation() {
  const [simState, setSimState] = useState({
    nsQueue: 0, ewQueue: 0, light: 0, cleared: 0, reward: 0,
    cumulativeReward: 0, isRunning: false, spawnRate: 0.4,
    simMode: 'intersection', lanes: 4, activeModel: 'Internal Q-Learning',
    trafficType: 'low', agentType: 'QL',
  });
  const [perfData, setPerfData]       = useState([]);
  const [socket, setSocket]           = useState(null);
  const [connected, setConnected]     = useState(false);
  ...
  // ── Socket ──────────────────────────────────────────────────────────────────
  useEffect(() => {
    // ✅ FIX: server runs on port 8000
    const s = io('http://localhost:8000', { withCredentials: true });

    s.on('connect', () => {
      setConnected(true);
      setSocket(s);
      // Send SetupAutomation config immediately on connect
      try {
        const raw = localStorage.getItem('automationConfig');
        if (raw) s.emit('update_config', JSON.parse(raw));
      } catch {}
      // Ask server for available trained models
      s.emit('get_models');
    });
    ...
    const tick = () => {
      ...
      setCars(prev => {
        ...
        if (st.isRunning && spawnTick.current % 50 === 0) {
          const rate       = st.spawnRate || 0.4;
          const totalLanes = st.lanes || 4;
          const dirs       = st.simMode === 'single' ? ['NS', 'SN'] : ['NS', 'SN', 'EW', 'WE'];
          dirs.forEach(dir => {
            if (Math.random() < rate * 0.5) {
              const blocked = next
                .filter(c => c.direction === dir && c.lane === Math.floor(Math.random() * totalLanes))
                .some(c => Math.abs(c.pos - START_POS[dir]) < CAR_H + CAR_GAP + 10);
              if (!blocked) next.push(makeCar(dir, totalLanes));
            }
          });
        }
        ...
        ['NS','SN','EW','WE'].forEach(dir => {
          ...
          lane.forEach(car => {
            ...
            if (leader) {
              const safeCentre = leader.pos - sign * (CAR_H / 2) - sign * (CAR_GAP + CAR_H / 2);
              maxPos = sign > 0 ? Math.min(maxPos, safeCentre) : Math.max(maxPos, safeCentre);
            }
  ...
  // ── Derived labels from config ───────────────────────────────────────────────
  const trafficLabel = simState.trafficType === 'high' ? '🔴 HIGH' : '🟢 LOW';
  const agentLabel   = simState.agentType === 'DQN' ? 'Deep Q-Network' : 'Q-Learning';
  ...
  return (
    <div className="cyber-container">
      ...
      <div className="dashboard-sidebar">
        <div className="dashboard-header">
          <h1 className="cyber-title">SMART CITY AI</h1>
          <p className="cyber-subtitle">{agentLabel} Traffic Control</p>
          <div className={`status-badge ${connected ? 'online' : 'offline'}`}>
            <div className="pulse-dot" />
            {connected ? 'LIVE CONNECTION' : 'DISCONNECTED'}
          </div>
        ...
        {/* Config summary from SetupAutomation */}
        <div className="glass-panel" style={{ padding: '1rem 1.5rem', marginBottom: '1rem' }}>
          <h3 className="panel-title" style={{ marginBottom: '0.8rem' }}>SESSION CONFIG</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.6rem', fontSize: '0.7rem' }}>
            {[
              ['MODE',    simState.simMode?.toUpperCase()],
              ['TRAFFIC', trafficLabel],
              ['AGENT',   simState.agentType || 'QL'],
              ['LANES',   simState.lanes || 4],
            ].map(([k, v]) => (
              <div key={k} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '8px', padding: '0.5rem 0.8rem' }}>
                <div style={{ color: '#6c7086', letterSpacing: '1px', marginBottom: '2px' }}>{k}</div>
                <div style={{ color: '#00f2fe', fontWeight: 700 }}>{v}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '1.2rem 1.5rem', marginBottom: '1rem' }}>
          <h3 className="panel-title">MODEL BRAIN</h3>
          {errorMsg && (
            <div style={{ background: 'rgba(255,61,0,0.1)', border: '1px solid rgba(255,61,0,0.3)',
              borderRadius: '8px', padding: '8px', fontSize: '0.7rem', color: '#ff3d00', marginBottom: '0.8rem' }}>
              ⚠ {errorMsg}
            </div>
          )}
          <select
            ...
            style={{
              ...
              border: '1px solid rgba(0,242,254,0.3)', borderRadius: '8px',
              color: '#fff', padding: '10px', fontSize: '0.8rem',
              outline: 'none', cursor: 'pointer',
            }}
          >
            {availableModels.map(m => (
              <option key={m.name} value={m.name} style={{ background: '#0a0a0f' }}>{m.name}</option>
            ))}
          </select>
          <div style={{ fontSize: '0.65rem', color: '#8A92A6', marginTop: '8px',
            display: 'flex', alignItems: 'center', gap: '5px' }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#00f2fe' }} />
            ACTIVE: <span style={{ color: '#00f2fe', fontWeight: 700 }}>
              {simState.activeModel?.toUpperCase()}
            </span>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-around' }}>
            {[['N ↕ S', nsGreen], ['E ↔ W', ewGreen]].map(([lbl, g]) => (
              <div key={lbl} style={{ textAlign: 'center' }}>
                <div style={{
                  width: 18, height: 18, borderRadius: '50%', margin: '0 auto 5px',
                  background: g ? '#00e676' : '#440000',
                  boxShadow: g ? '0 0 12px #00e676' : 'none', transition: 'all 0.4s'
                }} />
                <div style={{ fontSize: '0.7rem', color: '#8A92A6', letterSpacing: '1px' }}>{lbl}</div>
                <div style={{ fontSize: '0.78rem', fontWeight: 700,
                  color: g ? '#00e676' : '#ff1744', marginTop: 3 }}>
                  {g ? 'GREEN' : 'RED'}
                </div>
              </div>
            ))}
        ...
        <div className="glass-panel" style={{ padding: '1.2rem 1.5rem', marginBottom: '1rem' }}>
          ...
          <div style={{ marginTop: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between',
              fontSize: '0.6rem', color: '#8A92A6', marginBottom: '4px' }}>
              <span>AGENT EFFICIENCY</span>
              <span>{Math.max(0, Math.min(100, 100 + (simState.reward || 0) * 5)).toFixed(0)}%</span>
            </div>
            <div style={{ width: '100%', height: '4px', background: 'rgba(255,255,255,0.1)',
              ...
              <div style={{
                width: `${Math.max(0, Math.min(100, 100 + (simState.reward || 0) * 5))}%`,
                height: '100%', background: '#00f2fe', transition: 'width 0.4s ease'
              }} />
            </div>
          </div>
        </div>
        ...
        <div className="chart-panel glass-panel" style={{ height: 210, marginTop: '1rem', padding: '1rem' }}>
          ...
          <ResponsiveContainer width="100%" height="80%">
            <LineChart data={perfData}>
              ...
              <YAxis yAxisId="left"  stroke="#6c7086" fontSize={10} />
              <YAxis yAxisId="right" orientation="right" stroke="#6c7086" fontSize={10} />
              <Tooltip contentStyle={{ background: '#0a0a0f', border: '1px solid rgba(255,255,255,0.1)', fontSize: '0.8rem' }}
      ...
      <div className="simulation-canvas">
        <svg viewBox={`0 0 ${W} ${H}`} className="intersection-svg">
                ...
                rw > 12 && rh > 12 ? (
                  <g key={bi}>
                    ...
                    {rw > 28 && rh > 28 && Array.from({ length: Math.floor(rh/13) }).map((_,wi) =>
                      Array.from({ length: Math.floor(rw/13) }).map((_,wj) => (
                        <rect key={`${wi}-${wj}`}
                          x={rx+4+wj*13} y={ry+4+wi*13} width={7} height={8} rx={1}
                          fill="rgba(255,255,255,0.6)" stroke="rgba(0,0,0,0.07)" strokeWidth={0.5} />
                      ))
                    )}
                  </g>
          ...
          {simState.simMode === 'intersection' ? (
            <>
              <Pole x={IX1-28} y={IY1-8} green={ewGreen} />
              <Pole x={IX2+28} y={IY1-8} green={nsGreen} />
              <Pole x={IX1-28} y={IY2+8} green={nsGreen} />
              <Pole x={IX2+28} y={IY2+8} green={ewGreen} />
            </>
          ) : (
            <>
              <Pole x={IX2+28} y={IY1-8} green={nsGreen} />

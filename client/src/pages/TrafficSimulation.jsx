...
export default function TrafficSimulation() {
  ...
  useEffect(() => {
    ...
    s.on('sim_update', d => {
      setSimState(d);
      setPerfData(p => { const n = [...p, { t: new Date().toLocaleTimeString(), v: d.cleared }]; return n.length > 20 ? n.slice(1) : n; });
    });
    return () => s.close();
  }, []);
        ...
        if (st.isRunning && spawnTick.current % 50 === 0) {
          ...
          ['NS', 'SN', 'EW', 'WE'].forEach(dir => {
            if (Math.random() < rate * 0.5) {
              ...
              if (!blocked) next.push(makeCar(dir));
            }
          });
        }
      ...
      <div className="simulation-canvas">
        <svg viewBox={`0 0 ${W} ${H}`} className="intersection-svg">
          ...
          {Array.from({ length: 16 }).map((_, i) => (
            <rect key={`nsd${i}`} x={CX - 2} y={i * 56 + 6} width={4} height={28} rx={2} fill="rgba(255,255,255,0.5)" />
          ))}
          {/* EW dashes */}
          {Array.from({ length: 16 }).map((_, i) => (
            <rect key={`ewd${i}`} x={i * 56 + 6} y={CY - 2} width={28} height={4} rx={2} fill="rgba(255,255,255,0.5)" />
          ))}
          ...
          {/* ── CARS ── */}
          <g filter="url(#carShadow)">
            {cars.map(c => <CarShape key={c.id} car={c} />)}
          </g>
        </svg>
      </div>
    </div>
